...
# parsedatetime library doesn't work with numerical dates,
...
def get_date(date):
    ...
    if "-" in date:                                                           # if these characters exist in the string
        return dateparser.parse(date, date_formats=['%d-%m-%Y']).date()       # the date provided is numerical and use the 'dateparser' library
    elif "/" in date:
        return dateparser.parse(date, date_formats=['%d/%m/%Y']).date()           
    else:
